...
struct SetOfEigenValueVector QRMethod(double **matrix, double tolerance){
    ...
    setOfEigenValueVectorAnswer.eigenValues  = diagonalMatrix;

    return setOfEigenValueVectorAnswer;
}

double** DiagonalMatrixPow(double **matrix){
    for (int i = 0; i < lines ; ++i) {
        for (int j = 0; j < columns ; ++j) {
            if(i == j){
                matrix[i][j] = pow(matrix[i][j],10);
            }
        }
    }
    return matrix;
...
int main() {
    ...
    double  **matrixPow10 = MatrixAllocation(lines, columns);
    ...
    matrix = LUDecompositionForInverse(matrix);
    PrintMatrix(matrix);*/
    ...
    PrintMatrix(answerQr.eigenValues);
    printf("QrMethod eigenVectors: \n");
    PrintMatrix(answerQr.eigenVectors);

    matrixPow10 = DiagonalMatrixPow(answerQr.eigenValues);
    matrixPow10 = MatrixMultiplication(answerQr.eigenVectors, matrixPow10);
    matrixPow10 = MatrixMultiplication(matrixPow10, MatrixTranspose(answerQr.eigenVectors));

    printf("Matrix pow 10: \n");
    PrintMatrix(matrixPow10);
